"""Backtest the 3 live strategies against historical data before trusting
them with real trades.

Walks bars forward with no lookahead -- each decision only ever sees bars
up to and including "now" -- and reuses the exact same Strategy classes the
live bot runs (via bot.main.build_strategies()), so the backtest can never
silently drift from what's actually deployed. Applies 0.05% slippage and
$0 commission (Alpaca is commission-free) to every simulated fill.

Two deliberate deviations from the generic guide this was built from:
